...
- `gateway/` — messaging channels (`discord`, `telegram`, `slack`, `webchat`). `start_gateway()` wires up **all four** to the real agent loop (Slack is served by the WebChat axum server). Discord persists sessions/threads in `discord.db` and can route turns through the Claude Code CLI bridge (`src/core/claude_bridge.rs`); Telegram/Slack sessions are in-memory (lost on restart).
